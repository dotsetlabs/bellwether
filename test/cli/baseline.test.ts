...
describe('baseline command', () => {
  ...
  beforeEach(() => {
    ...
    testDir = join(
      tmpdir(),
      `bellwether-baseline-test-${Date.now()}-${Math.random().toString(36).slice(2)}`
    );
    ...
    vi.doMock('../../src/baseline/converter.js', () => ({
      createBaselineFromInterview: vi.fn().mockReturnValue({
        version: '1.0.0',
        metadata: {
          mode: 'check',
          generatedAt: new Date().toISOString(),
  ...
  describe('save subcommand', () => {
    ...
    it('should error when report file not found', async () => {
      ...
      await expect(baselineCommand.parseAsync(['node', 'test', 'save'])).rejects.toThrow(
        'Process exit: 4'
      );
    });
    ...
    it('should save baseline when report exists', async () => {
      ...
      writeFileSync(join(testDir, 'bellwether-check.json'), JSON.stringify(sampleInterviewResult));
  ...
  describe('show subcommand', () => {
    ...
    it('should error when baseline not found', async () => {
      ...
      await expect(baselineCommand.parseAsync(['node', 'test', 'show'])).rejects.toThrow(
        'Process exit: 4'
      );
    });
    ...
    it('should display baseline when found', async () => {
      ...
      writeFileSync(join(testDir, 'bellwether-baseline.json'), JSON.stringify(sampleBaseline));
  ...
  describe('diff subcommand', () => {
    ...
    it('should error when second baseline not found', async () => {
      ...
      writeFileSync(join(testDir, 'baseline1.json'), JSON.stringify(sampleBaseline));
    ...
    it('should compare two baselines when both exist', async () => {
      ...
      await baselineCommand.parseAsync([
        'node',
        'test',
        'diff',
        'baseline1.json',
  ...
  describe('accept subcommand', () => {
    ...
    it('should error when baseline not found', async () => {
      ...
      await expect(baselineCommand.parseAsync(['node', 'test', 'accept'])).rejects.toThrow(
        'Process exit: 4'
      );
    });
  });
...
describe('baseline file operations', () => {
  ...
  beforeEach(() => {
    testDir = join(
      tmpdir(),
      `bellwether-baseline-files-${Date.now()}-${Math.random().toString(36).slice(2)}`
    );
    mkdirSync(testDir, { recursive: true });
  });
