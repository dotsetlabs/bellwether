---
title: Output Formats
sidebar_position: 5
---

# Output Formats

Inquest supports multiple output formats for different use cases: documentation, CI/CD integration, and security scanning.

## Available Formats

| Format | File | Use Case |
|:-------|:-----|:---------|
| Markdown | `AGENTS.md` | Human-readable documentation |
| JSON | `inquest-report.json` | Machine-readable data |
| SARIF | `inquest.sarif` | GitHub Code Scanning |
| JUnit | `junit.xml` | CI test runners |

## Markdown (Default)

Human-readable behavioral documentation.

```bash
inquest interview npx your-server
# Output: AGENTS.md
```

### Example Output

```markdown
# @modelcontextprotocol/server-filesystem

> Generated by Inquest on 2026-01-12 using gpt-4o

## Overview

A file management server providing read/write access to the local filesystem.

## Tools

### read_file

Reads the contents of a file from the specified path.

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| path | string | yes | Path to the file |

**Observed Behavior:**
- Returns file contents as UTF-8 text
- Binary files returned as base64
- Maximum file size: 10MB

**Error Handling:**
- `ENOENT`: File not found
- `EACCES`: Permission denied

**Limitations:**
- Cannot read outside root directory

**Security Considerations:**
- Path traversal normalized within root
```

## JSON Report

Machine-readable format for programmatic access.

```bash
inquest interview --json npx your-server
# Output: inquest-report.json
```

### Example Output

```json
{
  "version": 1,
  "timestamp": "2026-01-12T10:30:00Z",
  "server": {
    "name": "@modelcontextprotocol/server-filesystem",
    "version": "1.0.0"
  },
  "tools": [
    {
      "name": "read_file",
      "description": "Reads file contents",
      "schema": {
        "type": "object",
        "properties": {
          "path": { "type": "string" }
        },
        "required": ["path"]
      },
      "interview": {
        "questionsAsked": 3,
        "observations": [...],
        "errors": [...],
        "security": [...]
      }
    }
  ],
  "cost": {
    "tokens": 1234,
    "estimatedCost": 0.02
  }
}
```

## SARIF (Security)

Static Analysis Results Interchange Format for GitHub Code Scanning.

```bash
inquest interview --output-format sarif -o ./results npx your-server
# Output: results/inquest.sarif
```

### Upload to GitHub

```yaml
# .github/workflows/security.yml
- name: Run Inquest Security Scan
  run: |
    inquest interview \
      --persona security_tester \
      --output-format sarif \
      -o ./results \
      npx your-server

- name: Upload SARIF
  uses: github/codeql-action/upload-sarif@v3
  with:
    sarif_file: ./results/inquest.sarif
```

### Example Output

```json
{
  "$schema": "https://raw.githubusercontent.com/oasis-tcs/sarif-spec/master/Schemata/sarif-schema-2.1.0.json",
  "version": "2.1.0",
  "runs": [
    {
      "tool": {
        "driver": {
          "name": "Inquest",
          "version": "0.2.0"
        }
      },
      "results": [
        {
          "ruleId": "security/path-traversal",
          "level": "warning",
          "message": {
            "text": "Potential path traversal vulnerability in read_file"
          }
        }
      ]
    }
  ]
}
```

## JUnit XML

For CI test runners that expect JUnit format.

```bash
inquest interview --output-format junit -o ./results npx your-server
# Output: results/junit.xml
```

### Use with GitLab CI

```yaml
inquest:
  script:
    - inquest interview --output-format junit -o ./results npx your-server
  artifacts:
    reports:
      junit: results/junit.xml
```

### Example Output

```xml
<?xml version="1.0" encoding="UTF-8"?>
<testsuites name="Inquest" tests="12" failures="1" time="45.2">
  <testsuite name="read_file" tests="3" failures="0">
    <testcase name="happy_path_read" time="2.1"/>
    <testcase name="error_handling_missing" time="1.8"/>
    <testcase name="security_path_traversal" time="3.2"/>
  </testsuite>
</testsuites>
```

## Multiple Formats

Generate multiple formats in one run:

```bash
inquest interview \
  --json \
  --output-format sarif \
  -o ./output \
  npx your-server
```

This creates:
- `output/AGENTS.md`
- `output/inquest-report.json`
- `output/inquest.sarif`

## Custom Output Directory

```bash
inquest interview -o ./docs npx your-server
# Output: docs/AGENTS.md
```

## See Also

- [CI/CD Integration](/guides/ci-cd) - Using outputs in pipelines
- [interview](/cli/interview) - Output options
- [Drift Detection](/concepts/drift-detection) - Comparing outputs
