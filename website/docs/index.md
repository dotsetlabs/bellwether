...
- **Cloud Sync** - Optional cloud storage for baseline history, team collaboration, and verification badges
- **Multiple Output Formats** - Markdown, JSON, JUnit XML, and SARIF for GitHub Code Scanning

## How It Works

```
   MCP Server           Inquest                    Output
       |                   |                          |
       |  tools/list       |                          |
       |<------------------|                          |
       |                   |                          |
...
Inquest generates `AGENTS.md` files documenting observed server behavior:
...
Inquest uses LLMs for intelligent testing. Typical costs per interview (10 tools, 3 questions each):
...
- [Installation](/installation) - Install Inquest and configure your LLM provider
